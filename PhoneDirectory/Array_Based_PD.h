...
private:
	...
	{
	private:
		std::string individual_name; // THE name of the individual represented in the entry
		std::string individual_number; // THE phone number for this individual
	public:
		...
		Directory_Entry(std::string the_name, std::string the_number) {
			this->individual_name = the_name; // INITIALIZE the constructor
			this->individual_number = the_number; // SET parameter values to member variables
		}
		std::string get_name() const {
			return this->individual_name; // RETRIEVES the name
		}
		std::string get_number() const {
			return this->individual_number; // RETRIEVES the number
		}
		void set_number(const std::string& new_number) {
			this->individual_number = new_number; // SETS the number to the specified value
		}
	private:
